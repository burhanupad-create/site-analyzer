...
export async function POST(
  ...
  // Snapshot values needed by the background task before clearing apiKey from job
  const { strategy, url: siteUrl, skippedReasons, truncated, totalRaw, apiKey } = job;

  // Clear the stored apiKey now that we've read it — don't keep it in Redis longer than needed
  await updateJob(jobId, { apiKey: undefined });
...
async function runPsiInBackground(opts: {
  ...
  try {
    await setJobRunning(jobId, `Starting analysis of ${selectedUrls.length} pages…`, 10);
