...
export function PagePicker({ jobId }: { jobId: string }) {
  ...
  // Fetch job data once on mount
  useEffect(() => {
    fetch(`/api/job/${jobId}`)
      .then((r) => r.json())
      .then((data: JobData & { error?: string }) => {
        ...
        if (data.status === "failed") {
          setFetchError(data.error ?? "Sitemap discovery failed");
          return;
        }
        setJobData(data);
        // Pre-check non-excluded URLs
        const preChecked = new Set<string>();
        for (const url of data.discoveredUrls) {
          if (!isExcludedUrl(url)) preChecked.add(url);
        }
        setSelected(preChecked);
      })
      .catch(() => setFetchError("Failed to load pages. Please try again."));
  }, [jobId, router]);
  ...
  async function handleStart() {
    if (!jobData || selected.size === 0) return;
    setStarting(true);
    setStartError("");
    ...
    try {
      ...
      setStartError(err instanceof Error ? err.message : "Unexpected error");
      setStarting(false);
    }
  }
  ...
  const urls = jobData.discoveredUrls;
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-sm">
          The sitemap crawl didn&apos;t find any pages to analyze.
        </p>
        <Link href="/"><Button variant="outline"><ArrowLeft className="h-4 w-4 mr-2" />Try another URL</Button></Link>
      </div>
    );
  ...
  const estimatedMins = Math.max(1, Math.ceil((selected.size * 8) / 60));
  const tooManySelected = selected.size > 30;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground pl-6">
          {jobData.url} · {urls.length} pages discovered
          {jobData.truncated && " (sitemap truncated)"}
        </p>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => setSelected(new Set(urls))}
      ...
      {tooManySelected && (
        <div className="flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800">
          ...
          <span>
            <strong>{selected.size} pages</strong> selected — analysis may take ~{estimatedMins} min.
            Consider selecting fewer pages (ideally under 30) for faster results.
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between pt-2 border-t">
        <p className="text-xs text-muted-foreground">
          ~{estimatedMins} min estimated · {selected.size} PSI {selected.size === 1 ? "call" : "calls"}
        </p>
        <Button onClick={handleStart} disabled={starting || selected.size === 0} size="lg">
          {starting ? (
